...
data class Propensione(
    ...
    val confermataDaiFatti: Boolean get() = partite >= PARTITE_MINIME_PER_ETICHETTA
    ...
    val etichetta: Ruolo?
        get() {
            ...
            val ordinati = valori.entries.sortedByDescending { it.value }
            val primo = ordinati[0].value
            val secondo = ordinati.getOrNull(1)?.value ?: 0.0
            if (primo <= 0.0) return null
            return if ((primo - secondo) / primo >= DISTACCO_MINIMO) ordinati[0].key else null
        }
    ...
    companion object {
         ...
         */
        fun diPartenza(ruolo: Ruolo): Propensione = Propensione(
