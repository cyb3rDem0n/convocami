...
if interfaccia.exists():
    ...
    contratti = {
        r"(?:CompositionLocalProvider\s*\(\s*LocalContentColor\s+provides\s+Gesso\s*\)|Surface\s*\([^)]*contentColor\s*=\s*Gesso)":
            "manca il colore chiaro predefinito: i testi sul fondo scuro tornano neri",
        r"painterResource\s*\(\s*R\.mipmap\.ic_launcher_foreground\s*\)":
            "il marchio dell'app non viene mostrato nella schermata di accesso",
        r'AzionePrimaria\s*\([^)]*testo\s*=\s*"APRI UNA PARTITA"[^)]*azione\s*=\s*nuovaPartita':
            "la CTA Apri una partita non e' collegata alla navigazione",
        r"Destinazione\.NuovaPartita\s*->\s*NuovaPartita\s*\(":
            "manca la destinazione del modulo nuova partita",
    }
    for modello, messaggio in contratti.items():
        if not re.search(modello, testo, re.DOTALL):
